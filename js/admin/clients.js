// ============================================================
// NOVANEST — Client website requests admin
// ============================================================
...
function render() {
  ...
  const filtered = allRows.filter((r) => {
    if (term) {
      const hay = `${r.name || ""} ${r.business_name || ""} ${r.email || ""}`.toLowerCase();
  ...
  if (!filtered.length) {
    container.innerHTML = `<div class="admin-empty"><strong>No requests found.</strong></div>`;
    return;
  }
  ...
  return `
    <article class="msg-card" data-id="${r.id}">
      <div class="msg-card__head">
        <div>
          <h3 class="msg-card__title">${escapeHtml(r.name || "")}</h3>
          <div class="msg-card__meta">
            ${r.business_name ? "🏢 " + escapeHtml(r.business_name) + " • " : ""}
            ${escapeHtml(r.email || "")}
      ...
      <div class="msg-card__kv">
        ${r.business_type ? `<div><strong>Type:</strong> ${escapeHtml(r.business_type)}</div>` : ""}
        ${r.service ? `<div><strong>Service:</strong> ${escapeHtml(r.service)}</div>` : ""}
        ${r.budget ? `<div><strong>Budget:</strong> ${escapeHtml(r.budget)}</div>` : ""}
      </div>

      ${r.description ? `<div class="msg-card__body">${escapeHtml(r.description)}</div>` : ""}

      <div class="msg-card__actions">
        <a href="mailto:${escapeHtml(r.email || "")}">Reply by Email</a>
        ${r.phone ? `<a href="https://wa.me/${escapeHtml(r.phone.replace(/\D/g, ""))}" target="_blank" rel="noopener">WhatsApp</a>` : ""}
...
async function handleClick(e) {
  ...
  const { error } = await supabase.from("client_inquiries").delete().eq("id", btn.dataset.id);
